...
func (s *Server) routes(mux *http.ServeMux, webFS fs.FS) {
	...
	mux.Handle("POST /network/usb/activate", authed(http.HandlerFunc(s.network.HandleUSBActivate)))
	mux.Handle("POST /network/usb/deactivate", authed(http.HandlerFunc(s.network.HandleUSBDeactivate)))
